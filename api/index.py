...
# Vercel ထဲကနေ Gemini API Key ကို လှမ်းယူပါမယ်
API_KEY = os.getenv("GEMINI_API_KEY", "AIzaSyCoJSpaJbmWJQylhOT-lhucsla2JBWz4os")
...
<head>
    ...
    <title>ANDERCO Progress Report</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-50 min-h-screen p-4 md:p-8">
    <div class="max-w-4xl mx-auto bg-white p-6 md:p-10 rounded-xl shadow-lg border border-slate-100">
        <h1 class="text-2xl md:text-3xl font-bold text-slate-800 mb-2 text-center">ANDERCO Site Progress Generator</h1>
        <p class="text-center text-slate-500 mb-8 text-sm">Powered by Google Gemini AI (Free Tier Optimized)</p>
        ...
        <button id="generate-btn" class="w-full mt-6 bg-indigo-600 text-white font-bold py-4 px-4 rounded-lg hover:bg-indigo-700 shadow-md hidden transition">
            Start AI Analysis & Generate PPTX
        </button>

        <div id="progress-container" class="mt-8 hidden">
            <div class="flex justify-between items-center mb-2">
                <p id="status-text" class="text-sm font-semibold text-slate-700">Initializing AI...</p>
    ...
    <script>
        ...
        generateBtn.addEventListener('click', async () => {
            ...
            for (let i = 0; i < selectedFiles.length; i++) {
                ...
                try {
                    ...
                    // Gemini Free API Limit မတက်အောင် တစ်ပုံနဲ့တစ်ပုံကြား ၃ စက္ကန့် ရပ်ပေးခြင်း
                    if (i < selectedFiles.length - 1) {
                        statusText.innerText = `Cooling down API (Avoiding Limit)...`;
        ...
        downloadBtn.addEventListener('click', () => {
            if (pptBlobUrl) {
                ...
                a.href = pptBlobUrl;
                a.download = "ANDERCO_Progress_Report.pptx";
...
async def analyze_photo(file: UploadFile = File(...)):
    ...
    STRICT RULES:
    1. Categorize the photo strictly into EXACTLY ONE of these 8 scopes: Earthworks & Site Clearance, Substructure & Foundation, Structural Works (Concrete & Formwork), Architectural & Masonry Works, Roofing & Waterproofing, M&E (Mechanical & Electrical) Works, Interior Finishes (Tiling, Painting, Carpentry), External Works & Landscaping
    2. Focus ONLY on the construction activities, materials, and stage of work.
    ...
    try:
        ...
        # PPT ထဲထည့်ရန် Base64 ပြောင်းခြင်း
        buffered = io.BytesIO()
        ...
        return {"filename": file.filename, "base64_image": "", "category": "Uncategorized", "caption": "Error processing image."}
...
async def generate_ppt(request: PPTXRequest):
    ...
    for cat, photos in grouped.items():
        ...
        for chunk in chunks:
            ...
            if num_photos == 1:
                ...
                positions = [(Inches(0.5), Inches(1.5), Inches(4)), (Inches(5.5), Inches(1.5), Inches(4)), (Inches(0.5), Inches(4.5), Inches(4)), (Inches(5.5), Inches(4.5), Inches(4))]
                
            for idx, photo in enumerate(chunk):
                left, top, width = positions[idx]
                
                if photo.base64_image:
    ...
    return Response(content=output_stream.getvalue(), media_type="application/vnd.openxmlformats-officedocument.presentationml.presentation", headers={"Content-Disposition": "attachment; filename=ANDERCO_Progress_Report.pptx"})
